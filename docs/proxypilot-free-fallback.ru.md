...
Система аварийного fallback с бесплатными AI-моделями на случай недоступности платных провайдеров (Anthropic). Используются 3 облачных провайдера + локальный Ollama как последнее средство.
...
3. При необходимости восстановить конфиг через flow из `docs/proxypilot-recovery.ru.md`
...
# 3. При необходимости восстановить развернутый конфиг ProxyPilot
# См. docs/proxypilot-recovery.ru.md
...
- Проверить наличие ключа через `gopass show -o api/<provider>`
- Проверить логи ProxyPilot: `journalctl --user -u proxypilot -f`
- Повторить recovery-flow из `docs/proxypilot-recovery.ru.md`

### Ключи пропадают после `just`

Доступ к секретам через gopass может зависеть от user-session unlock path, который недоступен в контексте Salt (root/sudo). AWK-фолбэк считывает ключи из уже развёрнутого конфига. Если ключи пропали:

1. Запустить flow из `docs/proxypilot-recovery.ru.md`, чтобы пересобрать `openai-compatibility`
...
| `docs/proxypilot-recovery.ru.md` | Flow восстановления и пересборки |
