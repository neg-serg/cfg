...
### 🔒 Запуск всего трафика через VPN (гибридная схема)

Для временного перенаправления **всего** трафика системы (включая графические приложения):

```bash
# Запустить с проверками и автооткатом
scripts/start-hybrid-vpn.sh

# Скрипт проверит:
# 1. Наличие бинарных файлов Xray и sing-box
# 2. Корректность конфигурационных файлов
# 3. Работоспособность SOCKS5-прокси
# 4. Создание TUN-интерфейса sb0
# 5. Корректность маршрутизации

# При ошибке на любом этапе выполняется автоматический откат
# При прерывании (Ctrl+C) все ресурсы очищаются
```

#### Проверка статуса:

```bash
# Проверить, работает ли гибридная схема
ip link show sb0 2>/dev/null && echo "TUN интерфейс активен" || echo "TUN неактивен"

# Проверить маршруты через VPN
ip route show table 200 2>/dev/null | head -5

# Полный статус (если скрипт запущен)
systemctl --user status vpn-split-router 2>/dev/null || echo "Гибридная схема работает напрямую"
```

#### Откат при проблемах:

Если после остановки скрипта остались проблемы с сетью:

```bash
# Полная очистка VPN-артефактов
