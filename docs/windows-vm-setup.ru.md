...
### Через `win-vm` (декларативный, воспроизводимый)

`~/.local/bin/win-vm` — это управляемый через конфиг менеджер жизненного цикла, обёртка над `virt-install`/`virt-xml`/`virsh`. Все параметры VM (RAM, CPU топология, ISO, GPU passthrough, Looking Glass, CPU pinning, hugepages, USB) хранятся в `~/.config/win-vm/config.sh` — отредактируйте один раз, пересоздайте одной командой.

```bash
win-vm init                        # создать ~/.config/win-vm/config.sh
$EDITOR ~/.config/win-vm/config.sh # установить VM_WINDOWS_ISO, настроить параметры
win-vm check                       # проверить пакеты, OVMF, libvirt socket
win-vm prepare                     # скачать virtio-win ISO, создать диск
win-vm define                      # создать libvirt VM из конфига
win-vm start                       # запустить VM
virt-viewer -c qemu:///system win11  # подключиться к консоли, запустить установку

# После завершения установки Windows:
win-vm eject-install               # отключить установочные ISO

# Опциональные пост-установочные настройки:
win-vm gpu-add                     # PCI passthrough RX 9070 XT (использует VM_GPU_*)
win-vm lg-enable                   # добавить IVSHMEM для Looking Glass
win-vm cpupin                      # привязать vCPU к ядрам хоста (использует VM_CPU_PIN_SET)
win-vm hugepages                   # включить память с hugepages
win-vm usb-add 0951:16ae           # горячее подключение USB-устройства
win-vm looking-glass               # запустить looking-glass-client
```

Жизненный цикл: `win-vm status`, `win-vm stop` (ACPI), `win-vm kill` (принудительно), `win-vm edit` (открывает `virsh edit`), `win-vm undefine`, `win-vm destroy-all` (удаляет VM + диск + nvram, запрашивает подтверждение).

Запустите `win-vm help` для полной справки.
