...
def fetch(url, dest):
    for attempt in range(3):
        try:
            subprocess.run(["curl", "-s", "--max-time", "120", "--socks5-hostname", "127.0.0.1:10808",
                url, "-o", dest], check=True, timeout=130)
            return True
...
def handle(client):
    ...
    try:
        ...
        if data and b'HTTP/' in data:
            try: client.send(b"HTTP/1.1 500\r\n\r\n")
            except: pass
    finally:
        try: client.close()
        except: pass
...
s.listen(200)
for _ in range(10):
    threading.Thread(target=lambda: [handle(c) for c in iter(lambda: s.accept(), None)], daemon=True).start()
