...
def check_network_connectivity(url: str = "https://httpbin.org/ip", timeout: int = 10) -> Tuple[bool, str]:
...
def create_emergency_domains_list(config: dict) -> bool:
    ...
    if not essential_domains:
        ...
        essential_domains = [
            ...
            "whatsapp.com", "discord.com", "telegram.org", "youtube.com",
            "twitch.tv", "reddit.com", "claude.ai", "openai.com",
            "chatgpt.com", "anthropic.com"
        ]
...
def check_and_correct_singbox_config(config: dict) -> bool:
    ...
    config_path_str = integration.get("config_path", "~/.config/sing-box-tun/config-no-auto-route.json")
    ...
    try:
        ...
        rkn_rule_exists = any(
            rule.get("tag") == "rkn-blocked-domains" 
            for rule in rules
        )
...
def run_autocorrection(config: dict, state: dict, force: bool = False) -> dict:
    ...
    if not healthy:
        ...
        max_failures = config.get("settings", {}).get("autocorrection", {}).get(
...
def command_status(args: argparse.Namespace) -> int:
    ...
    print(f"  VPN split router: {'OK' if state.get('integrations_ok', False) else 'Needs attention'}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="RKN Domains Autocorrection System"
    )
