...
def process_domains(content: str, config: dict) -> tuple[list[str], dict]:
    ...
    logger.info(f"Processed: {stats['valid_domains']} valid domains, {stats['filtered_out']} filtered out")
...
def integrate_with_vpn_split_router(domains: list[str], config: dict, state: dict) -> None:
    ...
    if integration.get("auto_mark_vpn", True):
        ...
        state["vpn_integration"] = {
            "updated": now_iso(),
            "seed_domains_count": len(current_seed)
        }
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="RKN Blocked Domains Fetcher with Autocorrection"
    )
    ...
    fetch_parser.add_argument("--integrate", action="store_true", help="Integrate with VPN split router")
