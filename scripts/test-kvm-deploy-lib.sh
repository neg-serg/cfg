...
_die() { echo "FATAL: ${1:-unknown error}" >&2; return 1; }
...
GRAINS
    ...
    # Enable systemd-networkd for DHCP (works out of the box)
    log_info "Configuring network with systemd-networkd..."
...
DHCP=yes
NETD
    cat > "$mnt/etc/systemd/network/50-en.network" <<'NETD2'
[Match]
Name=en*
[Network]
DHCP=yes
