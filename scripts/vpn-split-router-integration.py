...
def update_vpn_split_router_config(config_path: Path, domains: list[str]) -> bool:
    ...
    if config_path.exists():
        ...
        config = {
            ...
            'min_observations': 2,
            'vpn_domains_file': str(Path.home() / '.local' / 'state' / 'vpn-split-router' / 'vpn-domains.txt'),
            'observed_domains_file': str(Path.home() / '.local' / 'state' / 'vpn-split-router' / 'observed-domains.txt'),
            'state_file': str(Path.home() / '.local' / 'state' / 'vpn-split-router' / 'state.json'),
...
def main():
    ...
    parser.add_argument("--domains-file", default="~/.local/state/rkn-domains-fetcher/rkn-domains.txt",
                       help="RKN domains file path")
    parser.add_argument("--config-path", default="~/.config/vpn-split-router/config.yaml",
    ...
    if updated:
        print(f"\nTo apply changes:")
        print(f"  systemctl --user restart vpn-split-router")
        print(f"  Or run: python3 {Path.home() / 'src' / 'cfg' / 'scripts' / 'vpn_split_router.py'} --daemon")
    else:
