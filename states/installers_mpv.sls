...
{{ salt['installer.github_release_to']('mpv_script_' ~ (filename | replace('.', '_') | replace('-', '_')), filename, opts.repo, opts.asset, mpv_scripts_dir, tag=mpv_tag if mpv_tag else None, version=mpv_tag if mpv_tag else None, require='mpv_scripts_dir') }}
{% endfor %}
...
{% set _v = ver.get(name, '') %}
{{ salt['installer.github_release_to']('mpv_plugin_' ~ name, name, opts.repo, opts.asset, opts.dest, format='zip', tag=_v if _v else None, version=_v if _v else None, creates=mpv_scripts_dir ~ '/' ~ name, require='mpv_scripts_dir') }}
{% endfor %}
