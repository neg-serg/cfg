...
def _run_lib_function(func_name, *args):
    ...
    cmd = [
        "zsh",
        "-c",
        f'source "{lib_path}" 2>/dev/null; {func_name} {safe_args}; echo "EXIT:$?"',
    ]
...
def test_main_script_parseable():
    ...
    result = subprocess.run(
        ["zsh", "-n", str(SCRIPTS / "test-kvm-deploy.sh")], capture_output=True, text=True
    )
...
def test_main_script_help():
    ...
    result = subprocess.run(
        ["zsh", str(SCRIPTS / "test-kvm-deploy.sh"), "--help"], capture_output=True, text=True
    )
...
def test_library_check_rootfs_validates_paths():
    ...
    result = subprocess.run(
        [
            "zsh",
            "-c",
            f'source "{SCRIPTS}/test-kvm-deploy-lib.sh"; check_rootfs "/nonexistent/path"',
        ],
        capture_output=True,
        text=True,
    )
...
def test_library_resolve_profile_lists_all():
    ...
    result = subprocess.run(
        [
            "zsh",
            "-c",
            f'source "{SCRIPTS}/test-kvm-deploy-lib.sh"; resolve_profile "all" "{REPO_ROOT_PATH}"',
        ],
        capture_output=True,
        text=True,
...
def test_library_check_kvm_detects_presence():
    ...
    result = subprocess.run(
        ["zsh", "-c", f'source "{SCRIPTS}/test-kvm-deploy-lib.sh"; check_kvm'],
        capture_output=True,
        text=True,
    )
    assert result.returncode == 0
...
def test_library_check_prereqs():
    ...
    result = subprocess.run(
        ["zsh", "-c", f'source "{SCRIPTS}/test-kvm-deploy-lib.sh"; check_prereqs'],
        capture_output=True,
        text=True,
    )
